...
- 로컬/단일서버 compose 파일은 `process-gpt-infra-docker`(별도 레포)에 있다.
  이 스킬이 실행 중인 `process-gpt` 레포에는 `process-gpt-infra-docker/`가
  git 서브모듈로 등록되어 있으므로, 이미 `process-gpt`를 클론했다면 서브모듈만
  초기화하면 된다:
  ```bash
  git submodule update --init process-gpt-infra-docker
  cd process-gpt-infra-docker
  ```
  **도커 설치만 목적이고 `process-gpt` 소스는 필요 없다면** 이 레포 하나만
  단독으로 클론해도 된다 — `process-gpt-infra-docker`가 자체 `.gitmodules`로
