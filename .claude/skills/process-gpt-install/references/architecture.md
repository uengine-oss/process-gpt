...
| neo4j | 그래프 DB (실제로는 미사용 — 아래 참고) | 7474/7687 | **bpmn-extractor 전용** — 그 외 프로파일에선 생략 가능 |
| age-postgres | Apache AGE 그래프 DB (실제 사용되는 쪽) | 55433 | **bpmn-extractor + strategy 공유** — supabase/postgres엔 AGE가 없어 별도 인스턴스 (troubleshooting #37) |
| litellm-db / litellm-proxy | LLM 프록시 | `.env.example` 기본값 4010 | OpenAI 직결 시 우회됨 — unhealthy여도 무해 (troubleshooting #8) |

⚠️ **neo4j 컨테이너는 이름과 달리 실제로 안 쓰인다**: bpmn-extractor의
`Neo4jClient` 클래스는 이름만 Neo4j고 내부적으로 `psycopg.connect(AGE_DSN)`으로
**Apache AGE**(PostgreSQL 확장)에 붙는다. compose가 같이 띄우는 진짜
neo4j 서비스(`NEO4J_URI` 등)는 이 코드 경로에서 전혀 참조되지 않는 별개
컨테이너다 — 헷갈리기 쉬우니 주의.
...
| bpmn-extractor | PDF→BPMN | 8012 | **age-postgres 필요**(neo4j 컨테이너는 뜨지만 미사용, 위 참고) |
| process-gpt-office-mcp | 오피스 문서 MCP | 1192 | `LLM_PROVIDER: openai` env 필요 (#10) |
| mcp-validator | MCP 밸리데이터 | 8081 | 로컬에서 정상 빌드·기동되는 일반 서비스 |
| strategy | 전략 측정/설문(measure·survey) + 온톨로지 그래프 | 8014 | `/strategy-service/`로 nginx 라우팅됨, **age-postgres 필요**(bpmn-extractor와 공유) |
